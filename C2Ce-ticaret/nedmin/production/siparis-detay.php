...
$siparissor=$admindbservices->SiparisDetayGetir($_GET['kullanici_id'],$_GET['siparis_zaman']);
$kdvtoplam=0;
$aratoplam=0;
$toplam=0;
?>
          ...
          <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
           ...
           <tbody>
            ...
            while($sipariscek=$admindbservices->vericek($siparissor)) {
              ...
              $urunler=$cons->Urun_ekle($sipariscek);
              $toplam+=$sipariscek['satis_fiyat']*$siparisler->get_urun_adet();
              $kdvtoplam+=($sipariscek['satis_fiyat']*$urunler->get_urun_kdv())/100;
              ?>
              ...
              <tr>
                ...
                $tbl->addRow($siparisler->get_siparis_id(),1);
                $tbl->addRow($urunler->get_barkod_no(),2);
                $tbl->addRow($urunler->get_urun_ad(),3);

                $arraylistmarka=array();
                $markalist=new ArrayList($arraylistmarka);
                $markalist=$admindbservices->markaArrayListele($urunler->get_marka_id());
                $markalist=$markalist->toArray();
                foreach ($markalist as $markalarim) 
                {
                  $tbl->addRow($markalarim->get_marka_adi(),4);
                }

                $arraylistrenk=array();
                $renklist=new ArrayList($arraylistrenk);
                $renklist=$admindbservices->renkleriListele($urunler->get_renk_id());
                ...
                $bedenlerim=$admindbservices->SiparisBedenListele($urunler->get_beden_id());
                $tbl->addRow($bedenlerim->get_beden_icerik(),6);
                $tbl->addRow($siparisler->get_urun_adet(),7);
                $tbl->addRow(number_format($sipariscek['satis_fiyat'], 2, ',', '.'),8);
                $tbl->addRow(number_format($sipariscek['satis_fiyat']*$siparisler->get_urun_adet(), 2, ',', '.'),9);
